output$mylogo <- renderImage({
  list(src=findmypath("images","graphics_toolkat_smalllogo.png"),
                       width="300px",height="300px")
}, deleteFile = FALSE)

output$symbol_shapes <- renderImage({
  list(src=findmypath("images","symbol_shapes.png"),
...
for(x in c(1:7,"layer1","layer2","layer3")){
  local({  ## sans le "local" tous les outputs renvoient sur une meme image
    ...
    output[[paste0("catpaw",i)]]=renderImage({
      list(src=findmypath("images","catpaw.png"),width="18px",height="28px")
    },deleteFile=FALSE)
  })
}
